...
async function mqImportJSON(input){
  ...
  const newOnes=[];
  for(const r of data){if(!r.id||!r.name)continue;if(!mqRounds.find(x=>x.id===r.id)){r.ownerId=currentUser.uid;mqRounds.push(r);newOnes.push(r);}}
  if(newOnes.length>0){
    let failed=0;
...
async function qqImportJSON(input){
  ...
  try{const text=await file.text();const data=JSON.parse(text);if(!Array.isArray(data)){notify('Буруу формат');return;}
  const newOnes=[];
  for(const r of data){if(!r.id||!r.name)continue;if(!qqRounds.find(x=>x.id===r.id)){r.ownerId=currentUser.uid;qqRounds.push(r);newOnes.push(r);}}
  if(newOnes.length>0){
    let failed=0;
