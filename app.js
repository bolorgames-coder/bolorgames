...
function setAllInactive(){
  ...
  ['navHome','navMQ','navMV','navQQ','navMC','navFL','navLQ','navWQ','navAdminDash','navAdminLogos','navAdminWords'].forEach(id=>{const el=document.getElementById(id);if(el)el.classList.remove('active');});
...
function buildMoviePosterBG(){
  ...
  const posters=[
    {t:'CASABLANCA',y:'1942',top:6,left:2},{t:'THE GODFATHER',y:'1972',top:10,left:82},
    {t:'TITANIC',y:'1997',top:58,left:4},{t:'INCEPTION',y:'2010',top:70,left:78},
    {t:'PULP FICTION',y:'1994',top:30,left:90},{t:'FORREST GUMP',y:'1994',top:82,left:40},
    {t:'THE MATRIX',y:'1999',top:4,left:44},{t:'GLADIATOR',y:'2000',top:44,left:0}
  ];
  posters.forEach((p,i)=>{
    const d=document.createElement('div'); d.className='poster-item';
    d.style.top=p.top+'%'; d.style.left=p.left+'%'; d.style.animationDelay=(i*1.7)+'s';
    d.innerHTML='<div class="poster-reel">🎞</div><div class="poster-title">'+p.t+'</div><div class="poster-year">'+p.y+'</div>';
    wrap.appendChild(d);
  });
}
...
function mqStartRound(id){
  ...
  if(mqCurRound&&mqCurRound.type==='movie')document.body.classList.add('movie-mode');
  activeGame='mq';
}
  ...
  if(game==='mq'){
    if(q.audioData){
      ...
      if(ytMatch && round.type==='movie'){
        // ── MOVIE QUIZ: видео ИЛ ХАРАГДАНА ──
        const vid=ytMatch[1];
        // URL-д t= эсвэл start= параметр байвал тэр секундээс эхлүүлнэ
        ...
        ma.innerHTML=`<div class="video-box">
          <div style="position:relative;width:100%;padding-top:56.25%;border-radius:8px;overflow:hidden;background:#000;">
            <iframe src="https://www.youtube.com/embed/${vid}?autoplay=1&rel=0&modestbranding=1${startSec?'&start='+startSec:''}"
              style="position:absolute;inset:0;width:100%;height:100%;border:0;"
              allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>
          </div>
          <div style="margin-top:8px;font-size:11px;color:rgba(255,215,0,.55);font-family:'Share Tech Mono',monospace;">
            🎬 Видео нээгдэхгүй бол <a href="${escA(q.scUrl)}" target="_blank" rel="noopener" style="color:#ffd700;">YouTube дээр үзэх</a>
          </div>
        </div>`;
      }
      else if(ytMatch){
