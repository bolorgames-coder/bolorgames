...
import { getFirestore, collection, doc, setDoc, getDoc, getDocs, deleteDoc, query, orderBy, limit } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
  ...
  _loadAllInFlight=(async()=>{
    try{
      ...
      for(const d of mqSnap.docs){
        ...
        if(round.id && !seenMq.has(round.id)){seenMq.add(round.id); mqList.push(round);}
      }
      const qqList=[]; const seenQq=new Set();
      qqSnap.forEach(d=>{
        const round=d.data();
        if(round.id && !seenQq.has(round.id)){seenQq.add(round.id); qqList.push(round);}
      });
...
function setAllInactive(){
  ['navHome','navMQ','navQQ','navMC','navFL','navLQ','navWQ','navAdminDash','navAdminLogos','navAdminWords'].forEach(id=>{const el=document.getElementById(id);if(el)el.classList.remove('active');});
...
function mqGoHome(){ stopAudio(); showMQHome(); }
...
// ── NEW ROUND ──
let _nrGame=null;
function openNR(game){_nrGame=game;document.getElementById('nrInp').value='';document.getElementById('nrTitle').textContent=game==='mq'?'+ ШИНЭ MUSIC ТОГЛОЛТ':'⚛ ШИНЭ QUANTUM ТОГЛОЛТ';document.getElementById('nrOv').classList.add('open');}
...
function createRound(){
  ...
  if(_nrGame==='mq'){const r=mkMQRound(name);mqRounds.push(r);mqSaveOne(r).then(()=>{closeNR();mqRenderHome();openPlayerSetup('mq',r.id);}).catch(e=>{notify('Хадгалахад алдаа: '+String(e).slice(0,100));});}
...
async function mqImportJSON(input){
  ...
  let added=0;for(const r of data){if(!r.id||!r.name)continue;if(!mqRounds.find(x=>x.id===r.id)){mqRounds.push(r);added++;}}
  if(added>0){await mqSaveAll();mqRenderHome();notify(`${added} тоглолт import хийгдлээ!`);}else notify('Шинэ тоглолт олдсонгүй.');}
...
async function qqImportJSON(input){
  const file=input.files[0];if(!file)return;
  try{const text=await file.text();const data=JSON.parse(text);if(!Array.isArray(data)){notify('Буруу формат');return;}
  let added=0;for(const r of data){if(!r.id||!r.name)continue;if(!qqRounds.find(x=>x.id===r.id)){qqRounds.push(r);added++;}}
  if(added>0){await qqSaveAll();qqRenderHome();notify(`${added} тоглолт import хийгдлээ!`);}else notify('Шинэ тоглолт олдсонгүй.');}
  catch(e){notify('Алдаа!');console.error(e);}input.value='';
}
      ...
      if(secs%5===0 && secs>0){
        ...
        try{
          ...
          if(d.paid || d.payment_status==='PAID' || (d.rows && d.rows.length>0)){
            ...
            if(sd) sd.innerHTML=`Таны <b style="color:#00ff8c">${qpaySelectedMonths} сарын</b> subscription идэвхжлээ!<br>Одоо бүх тоглоомыг хязгааргүй тоглоорой.`;
