...
function cdBuildStage(round){
  ...
  for(let i=0;i<round.count;i++){
    ...
    const top=6+Math.random()*80, left=4+Math.random()*88;
...
function cdHostStartTimerBar(round, startedAt){
  ...
  const tick=()=>{
    ...
    if(bar) bar.style.width=pct+'%';
    if(elapsed>=round.showTime){ if(cover) cover.classList.add('show'); }
...
function cdHostRenderReveal(session){
  ...
  document.getElementById('cdHostActions').innerHTML = session.qIndex>=session.totalQ-1
    ? `<button onclick="cdHostEndGame()">🏁 Тэмцээн дуусгах</button>`
    : `<button onclick="cdHostNextQuestion()">Дараагийн раунд →</button>`;
...
function cdPlayerRunQuestionPhase(session){
  ...
  const tick=()=>{
    ...
    if(elapsed<round.showTime){
      ...
      if(inputTimerEl) inputTimerEl.textContent=Math.ceil(remain)+'с';
