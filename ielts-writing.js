...
`;
  ...
  function renderResult(d) {
    ...
    // Загвар заримдаа алдаагүй өгүүлбэрийг "засвар" болгон буцаадаг
    // (original === corrected). Тэдгээрийг харуулахгүй.
    var fixes = (d.corrections || []).filter(function (c) {
      var a = String(c.original || "").trim().replace(/\s+/g, " ");
      var b = String(c.corrected || "").trim().replace(/\s+/g, " ");
      return a && b && a.toLowerCase() !== b.toLowerCase();
    });

    if (fixes.length) {
      h += '<div class="iw-card"><div class="iw-h">Алдаа ба засвар</div>';
      fixes.forEach(function (c) {
  ...
  function mount(target, block, opts) {
    ...
    btn.addEventListener("click", async function () {
      ...
      try {
        ...
        // ⚠️ Загвар үг тоолж чаддаггүй тул wordCount-ыг өөрсдөө бодно
        data.wordCount = countWords(essay);
        data.meetsWordLimit = data.wordCount >= minWords;
